...
namespace UnitTestProject1
{
    ...
    public class FastDfsTest
    {
        ...
        private static readonly string tracker = ConfigurtaionHelper.Configuration["FastDFS:Tracker_server"];
        private static readonly int post = int.Parse(ConfigurtaionHelper.Configuration["FastDFS:Tracker_port"]);
        private static readonly string group = ConfigurtaionHelper.Configuration["FastDFS:Group"];
        private static readonly string storageLink = $"{ConfigurtaionHelper.Configuration["FastDFS:Storage_server"]}/{group}/";

        List<IPEndPoint> pEndPoints = new List<IPEndPoint>()
            {
                new IPEndPoint(IPAddress.Parse(tracker),post)
            };
        ...
        {
            StorageNode storageNode = FastDFSClient.GetStorageNodeAsync(group).GetAwaiter().GetResult();
            ...
            for (int i = 0; i < strArrays.Length; i++)
            {
                ...
                Console.WriteLine(storageLink + str);
                FastDFSClient.RemoveFileAsync(group, str).GetAwaiter().GetResult(); ;
                Console.WriteLine("FastDFSClient.RemoveFile" + str);
            }
        }
        ...
        private static async Task UploadAppendFile()
        {
            ...
            await FastDFSClient.AppendFileAsync(group, filename, appendBytes);
            ...
            await FastDFSClient.RemoveFileAsync(group, filename);
